...
if __name__ == '__main__':
    ...
    # Pertuebed Id
    bdry_chi = CompiledSubDomain('near((x[0]-1)*(x[0]+1), 0)')
    # bdry_chi = DomainBoundary()
    bdry_expr = Expression('near(x[0], -1) ? %s : 0' % ccode(bdry_motion).replace('M_PI', 'pi'),
    ...
    bcs = [DirichletBC(W.sub(0), bdry_velocity, 'near(x[0], -1)'),
